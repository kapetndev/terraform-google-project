locals {
  ...
  project_id  = var.project_id != null ? var.project_id : random_id.project_id.hex
}

resource "random_id" "project_id" {
  byte_length = 4
  prefix      = "${local.prefix}${var.name}-"
}
...
resource "google_project" "project" {
  ...
  name                = coalesce(var.descriptive_name, "${local.prefix}${var.name}")
